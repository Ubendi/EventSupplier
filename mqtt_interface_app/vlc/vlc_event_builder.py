...
class VLCEventBuilder:
    ...
    def select_subtitle_stream_id(self, language) -> dict[str, any]:
        subtitle_streams = self._get_streams("Subtitle", language)
        if not subtitle_streams:
            action = self.create_action("error", {"error_message" : f"No {language} subtitle found"})
        else:
            stream = subtitle_streams[0]
            action = self.create_action("set_subtitle", {"id" : stream[0], "language" : language})
        return action
    
    def select_audio_stream_id(self, language):
        audio_streams = self._get_streams("Audio", language)
        if not audio_streams:
            action = self.create_action("error", {"error_message" : f"No {language} audio found"})
        else:
            stream = audio_streams[0]
            action = self.create_action("set_audio", {"id" : stream[0], "language" : language})
        return action
    ...
    # Internal functions
    def _get_streams(self, type, language) -> list[tuple[str,dict]] | None:
        ...
        for stream in streams.items():
            ...
            if not lang:
                continue
            if normalize(lang) != normalize(language):
                continue
